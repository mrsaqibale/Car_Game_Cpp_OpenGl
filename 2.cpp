...
void drawHuman(float w, float z) {
glColor3f(0, 0, 0);
    glBegin(GL_POLYGON);
    for (int i = 0; i < 100; i++) {
        ...
        x = x/ 900.0;
        y = y/900.0;
        glVertex2f(w+(x+0.1), z+(y+0.0));  
        
        
    }
    glLineWidth(10);
        glBegin(GL_LINE_LOOP);
    glVertex2f(w + 0.05f, z - 0.05f);
    glVertex2f(w + 0.09f, z - 0.09f);
    // glVertex2f(w - 0.05f, z - 0.05f);
    // glVertex2f(w - 0.05f, z - 0.05f);
...
void display() {
    ...
    if (gameOver) {
        ...
        for (auto& human : humans) {
            drawHuman(human.posX, human.posY);
        }
...
void update(int value) {
    if (!gameOver) {
        ...
        // Spawn new humans
        if (rand() % 100 < 3) { // Adjust spawn rate here
            humans.push_back({ (rand() % 2 == 0)? -0.8f : 0.8f, (rand() % 10 + 5) / 10.0f, (rand() % 5 + 1) / 100.0f });
        }
